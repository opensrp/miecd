...
export interface DeforestAction extends AnyAction {
    ...
    treeByRootId: Record<string, never>;
}
 ...
 */
export function fetchTree(apiResponse: RawOpenSRPHierarchy, treeId: string | null = null): FetchedTreeAction {
 ...
 */
export function hierarchyReducer(state: ImmutableTreeState | Dictionary = initialState, action: TreeActionTypes) {
    switch (action.type) {
        case TREE_FETCHED:
            return {
                ...state,
 ...
 */
export const getRootJurisdictionId = (_: Partial<Store>, props: Filters) => props.rootJurisdictionId;
...
export const getNodesByNameOrId = () =>
    createSelector(geoLevelSelector, getSearchQuery, (nodes, searchQuery): TreeNode[] => {
        ...
        const matchesName = (node: TreeNode) => node.model.label.toLowerCase().includes(searchQuery.toLowerCase());
